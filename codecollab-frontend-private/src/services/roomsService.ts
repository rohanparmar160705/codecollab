...
export const listRooms = async () =>
  unwrap<any[]>(await api.get("/rooms")) ?? [];
export const getRoom = async (id: string) =>
  unwrap<any>(await api.get(`/rooms/${id}`));
...
export const deleteRoom = async (id: string) =>
  unwrap<any>(await api.delete(`/rooms/${id}`));
export const joinRoom = async (payload: { roomId: string; userId: string }) =>
...
export const saveRoomContent = async (payload: {
  roomId: string;
  content: string;
  language?: string;
}) =>
  unwrap<any>(
    await api.post(`/rooms/${payload.roomId}/content`, {
...
export const setRoomVisibility = async (id: string, isPublic: boolean) =>
  unwrap<any>(await api.put(`/rooms/${id}/visibility`, { isPublic }));
